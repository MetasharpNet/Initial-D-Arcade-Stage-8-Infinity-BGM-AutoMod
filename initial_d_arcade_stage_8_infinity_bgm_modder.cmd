...
title Initial D Arcade Stage 8 Infinity - BGM Modder
...
set "CONFIG_FILE=%SCRIPT_DIR%initial_d_arcade_stage_8_infinity_bgm_modder.txt"
...
cls
echo ============================================================
echo 🚗 Initial D Arcade Stage 8 Infinity BGM Modder 🎵
echo ============================================================
echo.
...
echo Found preview replacement WAV files: !NEW_PREV_COUNT!
...
for %%D in ("!NEW_WAV_DIR!" "!NEW_MUSICS_DIR!") do (
    if not defined PREVIEW_SOURCE (
        if exist "%%~D" (
            for %%F in ("%%~D\!TRACK_NUM! *.*") do (
                if not defined PREVIEW_SOURCE if exist "%%~fF" set "PREVIEW_SOURCE=%%~fF"
            )
        )
    )
)
...
if not defined PREVIEW_SOURCE (
    if exist "!NEW_WAV_DIR!\!MAIN_FILE!" set "PREVIEW_SOURCE=!NEW_WAV_DIR!\!MAIN_FILE!"
)

if defined PREVIEW_SOURCE (
    "!FFMPEG!" -y -i "!PREVIEW_SOURCE!" -t 50 "!NEW_PREV_DIR!\!PREV_FILE!" >nul 2>nul
    if errorlevel 1 (
        echo ⚠️ Failed to create 50s preview: !PREV_FILE!
    ) else (
        echo ✅ Created 50s preview !PREV_FILE!
    )
) else (
    echo ⚠️ Missing track !TRACK_NUM! / !MAIN_FILE! - preview not generated.
)
exit /b 0
...
>> "!SONGS_PS1!" echo for ($i = 1; $i -le 19; $i++) {
